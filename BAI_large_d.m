...
d = 10
K = 50
T_all = floor(10.^linspace(3.5, 5.5, 15));
...
[X, theta_s, best, Hardness, v_star, xmax2, B, Hardness_LB] = generate_instance(d, K, s, mode, 750, 1000);
Hardness
...
%  Lasso-OD-Analytical
    ...
    for run = 1:runtimes
        run
        est_best_bayesadaptive(run, :) = BayesGap_Adaptive_allT(theta_s, X, T_all(8), 20);
    end
    toc
    prob_error_BayesGapAdaptive = mean(est_best_bayesadaptive ~= best, 1);
    std_prob_error_BayesGapAdaptive = sqrt(prob_error_BayesGapAdaptive .* (1 - prob_error_BayesGapAdaptive) ./ (runtimes));

for t_index = 2:length(T_all)
...
lg = legend(ax, 'ODLinBAI', 'Lasso-OD', 'GSE', 'Box', 'off', 'NumColumns', 3)
