function best_arm = BayesGap_Adaptive_allT(theta, A, T_all, eta)
...
J_T = zeros(1, T);
for t = 1 : T
    theta_hat = Sigma \ theta_tmp;
    mu_t = theta_hat' * A;
    sigma_t = zeros(1, K);
    for ii = 1 : K
        sigma_t(ii) = sqrt(A(:, ii)' * (Sigma \ A(:, ii)));
    end
...
for t_index = 1:length(T_all)
    [~, I] = min(B_T(1:T_all(t_index)));
    best_arm(t_index) = J_T(I);
end
